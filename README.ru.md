...
<sup>**v4.4.2** · 18 августа 2026</sup>
...
[![Версия](https://img.shields.io/badge/версия-4.4.2-5fd0e2?style=flat-square)]()
...
## v4.4.2 · 18 августа 2026

- **CLIP SHAPE стал тёплым.** Прежняя кривая была симметричной, а симметричная умеет
  давать только резкие, «стеклянные» призвуки — отсюда и бралось ощущение лишнего верха.
  Теперь она работает как ламповый каскад: появляется мягкая окраска, а резкость при
  этом измеримо *уменьшается*. Громкость по-прежнему не меняется
- **DASH крутится ровно по всему ходу.** Раньше почти весь эффект сидел в первой
  четверти, а дальше ручка почти не отвечала. Теперь каждый поворот даёт одинаковую
  прибавку
- **Ручка PUNCH убрана.** Замеры показали, что при любом значении, кроме нуля, она в
  точности повторяла выключенное состояние — то есть не делала ничего. Ровностью теперь
  занимается один DASH. Проекты, сохранённые с PUNCH, открываются и звучат как прежде
- **CHARACTER больше не исчезает в режиме HOUZY.** Ручка остаётся на месте приглушённой,
  чтобы было видно: она есть, просто в этом движке не работает
- **Главная ручка AMOUNT крутится плавнее и точнее**
